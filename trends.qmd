...
```{r}
# trend symptom load
dat_NBMP_P14 <- dat_aggregated %>%
  filter(pseudonym == "NBMP-P14")

ggplot(dat_NBMP_P14, aes(x = query_number, y = A1b)) +
  geom_line(color = "darkblue", size = 0.5) +
  labs(title = "Trend Symptom Load NBMP-P14",
       x = "Query",
       y = "Mean Symptom Load") +
  theme_minimal()

# Multiple trends
overall_trends_NBMP_P14_long <- dat_NBMP_P14 %>%
  select(query_number, A1b, A2, A3) %>%
  pivot_longer(cols = starts_with("A"),
               names_to = "variable",
               values_to = "mean_value",
...
ggplot(overall_trends_NBMP_P14_long, aes(x = query_number, y = mean_value, color = variable)) +
  geom_line(size = 0.2) +
  geom_smooth(method = "loess", alpha = 0.05, size = 0.5) +
  scale_color_brewer(palette = "Set1") +
  labs(title = "Overall Trends NBMP-P14",
       x = "Query",
       y = "SCore",
       color = "Variable") +
  theme_minimal() +
...
# Linear trends
fit_trend_NBMP_P14 <- lm(A1b ~ query_number, data = dat_NBMP_P14)
summary(fit_trend_NBMP_P14)
...
plot_trends(data = dat_aggregated, variables = act_scores, output_dir = "plots/trends")
```
...
```{r}
plot_trends_days <- function(data, variables, output_dir = "plots") {
  ...
  # one file for all plots
  pdf(file.path(output_dir, "trends_all_participants_days.pdf"), width = 12, height = 14)
  ...
  plots <- lapply(variables, function(var) {
    ...
    # Aggregate over study days for this participant and variable
    dat_by_day <- dat_p %>%
      filter(interactionStatus == "Completed") %>%
      group_by(study_day) %>%
      summarise(
        mean_var = mean(.data[[var]], na.rm = TRUE),
        se_var   = sd(.data[[var]], na.rm = TRUE) / sqrt(n()),
        .groups  = "drop"
      )

    ggplot(dat_by_day, aes(x = study_day, y = mean_var)) +
      geom_point(color = "darkblue", size = 0.5) +
      geom_line(color = "lightblue", size = 0.5) +
      geom_vline(xintercept = seq(7, by = 7, length.out = 8),
                 colour = "grey", linetype = "longdash") +
      labs(title = var, x = "Study Day", y = paste("Mean", var)) +
      ylim(0, 100) +
      theme_minimal()
  })
    ...
    # Combine plots into grid
    combined <- wrap_plots(plots, ncol = 2) +
      plot_annotation(title = paste("Trend Overview by day -", pseudo))
...
plot_trends_days(data = dat_aggregated, variables = act_scores, output_dir = "plots/trends")
```
